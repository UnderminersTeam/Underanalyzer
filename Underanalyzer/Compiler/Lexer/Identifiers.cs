...
internal static class Identifiers
{
    ...
    /// <summary>
    /// Parses an internal identifier from the given text position.
    /// </summary>
    /// <remarks>
    /// An "internal" identifier is one starting with '@' and containing at least one character that is not '@'.
    /// </remarks>
    public static int ParseInternal(LexContext context, int startPosition)
    {
        int pos = ContiguousTextReader.ReadWhileInternalIdentifier(context.Text, startPosition, out ReadOnlySpan<char> identifier, out bool anyNormalChars);

        // Make sure at least 1 "normal" character showed up in the identifier
        if (!anyNormalChars)
        {
            context.CompileContext.PushError("Unrecognized token", context, pos);
        }
        else
        {
            context.Tokens.Add(new TokenIdentifier(context, startPosition, identifier.ToString()));
